import { Scene, WebGLRenderer, GridHelper, DirectionalLight, AmbientLight, Raycaster, Vector2,AxesHelper, MeshLambertMaterial } from "three";
...
const humP = document.getElementById("hum");
function iotDataTooltip(event,iotData, model,){
  const isFound = casting(event)[0];
  if (isFound){
    model.id = isFound.object.modelID;
    ...
    if(id){
          tempP.innerHTML = iotData.temp;
          humP.innerHTML = iotData.humi;
...
window.onclick = (event) => iotDataTooltip(event , barData, modelHover);
